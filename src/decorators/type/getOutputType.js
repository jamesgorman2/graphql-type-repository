...
function getList(
  ...
  if (typeNode.kind === 'ListType') {
    return getNonNull(typeNode.type, typeMap, referringTypeName, module)
      .orSome(() => getNamedOuputType((typeNode: any).type, typeMap, referringTypeName, module))
...
function getNamedOuputType(
  typeNode: TypeNode,
  typeMap: TypeMap,
  referringTypeName: string,
  module: Module
): GraphQLOutputType {
  return typeMap.getOuputType((typeNode: any).name.value, referringTypeName, module);
}
...
export function getOutputType(
  ...
  const t: GraphQLOutputType =
    getList(typeNode, typeMap, referringTypeName, module)
      ...
      .getOrElse(() => getNamedOuputType(typeNode, typeMap, referringTypeName, module));
  return t;
}
